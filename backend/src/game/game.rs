use crate::game::card::deck::Deck;
use crate::game::player::Player;
use shared::Card;
use shared::moment::Moment;
use crate::game::action::Action;
use crate::game::hand::calculate::HandCalculate;
use crate::game::hand::hand::Hand;
use serde::Serialize;

#[derive(Clone, Serialize)]
pub struct Game {
    pub players: Vec<Player>,
    pub current_player: usize,
    pub dealer_index: usize,
    pub global_turn: usize,
    pub pot: i32,
    pub max_bet: i32,
    pub current_blind: i32,
    pub deck: Deck,
    pub common_card: Vec<Card>,
    pub moment: Moment,
    pub started: bool,
    pub last_winner: Option<String>,
    pub run_out: bool,
}

impl Game {
    pub fn new() -> Game {
        Game {
            players: Vec::new(),
            current_player: 0,
            dealer_index: 0,
            global_turn: 0,
            pot: 0,
            max_bet: 0,
            current_blind: 1,
            deck: Deck::new(),
            common_card: Vec::new(),
            moment: Moment::Preflop,
            started: false,
            last_winner: None,
            run_out: false,
        }
    }

    // Game Data
    pub fn add_player(&mut self, player: Player){
        self.players.push(player)
    }

    pub fn remove_player(&mut self, player: Player) -> Option<Player> {
        if let Some(pos) = self.players.iter().position(|p| *p == player){
            Some(self.players.remove(pos))
        }
        else{
            None
        }
    }

    fn is_playable(&self, idx: usize) -> bool {
        let p = &self.players[idx];
        p.active && !p.folded && !p.is_all_in()
    }

    fn next_acting_from(&self, start: usize) -> Option<usize> {
        let size = self.players.len();
        for step in 1..=size {
            let idx = (start + step) % size;
            if self.is_playable(idx) {
                return Some(idx);
            }
        }
        None
    }

    pub fn next_player(&mut self){
        if self.players.len() <= 1 { return; }
        if let Some(next) = self.next_acting_from(self.current_player) {
            self.current_player = next;
        }
    }

    pub fn set_first_player(&mut self){
        if let Some(next) = self.next_acting_from(self.dealer_index) {
            self.current_player = next;
        }
    }

    pub fn incr_turn(&mut self){
        self.global_turn += 1;
        self.get_blind_level()
    }

    pub fn next_moment(&mut self){
        self.moment = self.moment.next();
    }

    pub fn action_moment(&mut self){
        match self.moment{
            Moment::Preflop => {
                self.pre_flop()
            },
            Moment::Flop => {
                self.flop()
            },
            Moment::Turn => {
                self.turn()
            },
            Moment::River => {
                self.river()
            }

        }
    }

    pub fn pre_flop(&mut self){
        self.next_button();
        self.clean_cards();
        for player in &mut self.players {
            if player.active{
                player.add_card(self.deck.draw().unwrap());
                player.add_card(self.deck.draw().unwrap());
            }
        }
    }

    pub fn flop(&mut self){
        self.deck.draw();
        for _ in 0..3 {
            self.common_card.push(self.deck.draw().unwrap());
        }
    }

    pub fn turn(&mut self){
        self.deck.draw();
        self.common_card.push(self.deck.draw().unwrap())
    }

    pub fn river(&mut self){
        self.deck.draw();
        self.common_card.push(self.deck.draw().unwrap())
    }


    // Run Work

    pub fn start(&mut self){
        self.clean_cards();
        self.last_winner = None;
        self.run_out = false;
        self.pot = 0;
    }

    pub fn clean_cards(&mut self){
        self.deck.reload();
        self.deck.shuffle();
        self.common_card.clear();
    }

    pub fn is_end(&self) -> bool {
        let mut count: usize = 0;
        for player in &self.players{
            if !player.folded && player.active && player.bankroll != 0{
                count += 1
            }
        }
        count == 1
    }

    pub fn end(&mut self){
        for player in &mut self.players{
            player.restore();
            if player.active && player.bankroll == 0{
                player.set_active(false)
            }
        }
        self.moment = Moment::Preflop;
        self.run_out = false;
    }

    pub fn run_out_active(&self) -> bool {
        self.run_out && !self.player_needs_to_play()
    }

    pub fn check_finished(&mut self) -> bool{
        let mut count: usize = 0;
        for player in &self.players {
            if player.active{
                count += 1;
            }
        }
        count == 1
    }

    pub fn all_talked(&self) -> bool{
        for player in &self.players{
            if player.active && !player.talked && !player.folded && !player.is_all_in(){
                return false
            }
        }
        true
    }

    pub fn untalked_all(&mut self){
        for player in &mut self.players{
            player.set_talked(false);
        }
    }

    pub fn bet_gather(&mut self){
        self.max_bet = 0;
        for player in &mut self.players{
            player.add_total_bet(player.bet);
            self.pot += player.bet;
            player.reset_round_stat()
        }
    }

    pub fn distribute_pots(&mut self) {
        let live: Vec<usize> = self.players.iter().enumerate()
            .filter(|(_, p)| !p.folded && p.active)
            .map(|(i, _)| i)
            .collect();

        if live.len() == 1 {
            let w = live[0];
            self.players[w].add_bankroll(self.pot);
            println!("{} win the {}$ pot !", self.players[w].name, self.pot);
            self.last_winner = Some(self.players[w].name.clone());
            return;
        }

        let results: Vec<(usize, Hand)> = live.iter().map(|&i| {
                let p = &self.players[i];
                let mut c = p.hand.cards.clone();
                c.extend(self.common_card.iter().cloned());
                let mut hc = HandCalculate::new(Hand::new(c));
                (i, hc.best_hands())
            }).collect();

        let mut bets: Vec<(usize, i32)> = self.players.iter().enumerate().map(|(i, p)| (i, p.total_bet)).collect();
        bets.sort_by_key(|&(_, b)| b);

        let mut prev = 0;

        for (rank, &(_, bet)) in bets.iter().enumerate() {

            if bet <= prev { continue; }
            let step = bet - prev;

            let contributors = self.players.iter().filter(|p| p.total_bet >= bet).count() as i32;
            let amount = step * contributors;

            let eligible: Vec<usize> = bets[rank..].iter().map(|&(j, _)| j).filter(|&j| !self.players[j].folded && self.players[j].active).collect();

            if eligible.is_empty() {
                prev = bet;
                continue;
            }

            let best = eligible.iter()
                .map(|&i| results.iter().find(|&(j, _)| *j == i).unwrap())
                .reduce(|a, b| if b.1.better_than(&a.1){ b } else { a }).unwrap();

            let winners: Vec<&usize> = eligible.iter()
                .filter(|&&i| {
                    let h = &results.iter().find(|&(j, _)| *j == i).unwrap().1;
                    !h.better_than(&best.1) && !best.1.better_than(h)
                }).collect();

            let share = amount / winners.len() as i32;
            for (i, &&w) in winners.iter().enumerate(){
                let extra = if (i as i32) < amount % winners.len() as i32 { 1 } else { 0 };
                self.players[w].add_bankroll(share + extra);
                println!("{} win the {}$ pot !", self.players[w].name, share + extra);
                self.last_winner = Some(self.players[w].name.clone());
            }
            prev = bet;
        }
    }


    fn next_button(&mut self){
        for player in &mut self.players{
            if player.active{
                player.restore()
            }
        }
        let size: usize = self.players.len();
        loop{
            self.dealer_index = (self.dealer_index + 1) % size;
            if self.players[self.dealer_index].active{
                break
            }
        }

        let heads_up = self.player_remaining().len() == 2;
        let sb = if heads_up { self.dealer_index } else { self.next_acting_from(self.dealer_index).unwrap_or(self.dealer_index) };
        self.players[sb].set_small_blind(true, self.current_blind);

        let bb = self.next_acting_from(sb).unwrap_or(sb);
        self.players[bb].set_big_blind(true, self.current_blind);
        self.max_bet = self.current_blind * 2;

        self.current_player = if heads_up {
            sb
        } else {
            self.next_acting_from(bb).unwrap_or(bb)
        };
    }

    pub fn get_blind_level(&mut self){
        let blind = (1.15_f64).powi(self.global_turn as i32) as i32;
        self.current_blind = blind.clamp(1,50)
    }

    pub fn apply_action(&mut self, action: Action) -> bool{
        match action {
            Action::Fold => {
                self.players[self.current_player].set_folded(true);
                true
            }

            Action::Check => {
                self.check()
            }

            Action::Call => {
                self.call()
            }

            Action::Raise(value) => {
                self.raise(value)
            }

            Action::AllIn => {
                self.raise(self.players[self.current_player].bankroll - self.players[self.current_player].bet)
            }
        }
    }

    pub fn check(&mut self) -> bool{
        if self.players[self.current_player].bet == self.max_bet{
            self.players[self.current_player].set_talked(true);
            return true
        }
        println!("Impossible check, pot != max_bet !");
        false
    }

    pub fn call(&mut self) -> bool{
        let gap = self.max_bet - self.players[self.current_player].bet;
        if gap < 0{
            return false
        }
        self.players[self.current_player].add_bet(gap);
        self.players[self.current_player].set_talked(true);
        true
    }

    pub fn raise(&mut self, raise_value: i32) -> bool{
        if raise_value <= 0{
            println!("Not a positive value !");
            return false
        }
        let call_value = self.max_bet - self.players[self.current_player].bet;
        let total_raise = call_value + raise_value;

        if self.players[self.current_player].bankroll >= total_raise{
            self.players[self.current_player].add_bet(total_raise);
            self.max_bet = self.players[self.current_player].bet;
            self.untalked_all();
            self.players[self.current_player].set_talked(true);
            return true
        }
        else{
            self.players[self.current_player].all_in();
            self.max_bet = self.players[self.current_player].bet.max(self.max_bet);
            self.untalked_all();
            self.players[self.current_player].set_talked(true);
        }
        true
    }

    pub fn player_remaining(&self) -> Vec<usize>{
        let mut player_list = Vec::new();
        for index in 0..self.players.len(){
            if !self.players[index].folded && self.players[index].active{
                player_list.push(index)
            }
        }
        player_list
    }

    pub fn player_needs_to_play(&self) -> bool{
        !self.is_end() && !self.all_talked()
    }

    pub fn tournament_winner(&self) -> bool {
        self.players.iter().filter(|p| p.active).count() == 1
    }

    pub fn advance(&mut self) {
        loop{
            let live = self.players.iter().filter(|p| p.active && !p.folded).count();
            if live <= 1 {
                self.run_out = false;
                self.bet_gather();
                self.distribute_pots();
                self.end();
                if !self.tournament_winner() {
                    self.start_round();
                }
                break;
            }
            if self.player_needs_to_play(){
                break;
            }
            if self.all_talked() {
                self.bet_gather();
                if self.moment == Moment::River {
                    self.run_out = false;
                    self.distribute_pots();
                    self.end();
                    if !self.tournament_winner() {
                        self.start_round();
                    }
                    break;
                }
                self.next_moment();
                self.set_first_player();
                self.action_moment();
                self.run_out = true;
                break;
            }
            else {
                break;
            }
        }
    }

    pub fn start_round(&mut self){
        self.incr_turn();
        self.start();
        self.action_moment();
        self.started = true;
    }

    pub fn step_run_out(&mut self) -> bool {
        if self.moment == Moment::River {
            self.run_out = false;
            self.distribute_pots();
            self.end();
            if !self.tournament_winner() {
                self.start_round();
            }
            return true;
        }
        self.next_moment();
        self.set_first_player();
        self.action_moment();
        false
    }
}